...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        space)
        state is a triple: (position, list of remaining corners, cost to start state)
        """
        "*** YOUR CODE HERE ***"
...
def cornersHeuristic(state, problem):
    ...
    for xy2 in corners:
        if xy2 not in visited:
            ans = min(ans, abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1]))

    return ans + (4-len(visited)-1) * max(min(walls.height-2, walls.width-2), 1)
